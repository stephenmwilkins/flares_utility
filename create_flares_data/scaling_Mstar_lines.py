...
for line in lines:
    ...
    for line_ in line:
        quantities.append({'path': f'Galaxy/BPASS_2.2.1/Chabrier300/Lines/DustModelI/{line_}',
                           'dataset': 'Luminosity', 'name': 'L'+line_, 'log10': False})
        quantities.append({'path': f'Galaxy/BPASS_2.2.1/Chabrier300/Lines/DustModelI/{line_}',
                           'dataset': 'EW', 'name': 'EW'+line_, 'log10': False})
    ...
    for q in quantiles:
        t.add_column(Column(data=np.round(
            lum_[q], 2), name=f'{line_id}_log10L_P{q*100:.1f}', unit='dex(erg/s)', description=rf'log_{{10}}(L_{{ {line} }}/erg\ s^{{-1}})'))
        t.add_column(Column(data=np.round(
            ew_[q], 2), name=f'{line_id}_EW_P{q*100:.1f}', unit='dex(AA)', description=rf'log_{{10}}(EW_{{ {line},0 }}/\AA)'))
    ...
    t.write(
        f'/Users/stephenwilkins/Dropbox/Research/projects/flares/flares_data/flares_data/data/scaling_relations/log10Mstar/{line_id}.ecsv', format='ascii.ecsv', overwrite=True)
