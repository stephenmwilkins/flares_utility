...
for add_flares in [True, False]:
    ...
    for z in [6.0, 10.0]:
        ...
        for simID, ls, alpha, c, label in zip(sims,['--','-'], [0.1, 0.2], ['0.5','k'],['EAGLE\ REF', 'FLARES']):
        ...
        for Survey in Surveys.keys():
            ...
            for subSurvey, SS in Surveys[Survey].items():
                ...
                ax.fill_between([M, -30], [low_lim, low_lim], alpha=0.1, label = fr'$\rm {{\bf {Survey} }}/{subSurvey}$', color=c)
        ...
        axH.set_xlim(M_limit)
        axH.set_ylim([-0.1, 4.5])
        axH.set_xticks([])
        ...
        ax.text(-23., -3, fr'$\rm z={z}$', fontsize = 12)

        if add_flares:
            fig.savefig(f'figures/LF_{z}.pdf')
        else:
            fig.savefig(f'figures/LF_{z}_NOFLARES.pdf')
        fig.clf()
