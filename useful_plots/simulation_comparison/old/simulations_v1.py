...
simulations['FLARES-1'] = {'size': (40*(4/3)*np.pi*14**3)**(1/3), 'DM_mass': 9.7E6, 'resimulation': 3200,  'RT': False}
...
my_simulations = ['FLARES-1', 'FLARES-HR']
...
for simulation_name in other_simulations:
    ...
    ax.text(3.8, 10.8-i*0.4, rf'$\rm{{\bf{i}}}:{simulation_name}$', fontsize = 8, ha = 'left', va = 'center', alpha = alpha)
...
if include_FLARES:
    for simulation_name in my_simulations:
        ...
        ax.text(res-0.15, volume-0.05, rf'$\rm\bf {simulation_name}$', fontsize = 10, ha = 'left', va = 'center', alpha = alpha,zorder = 6)
...
# ax1 = fig.add_axes([0.81, 0.1, 0.03, 0.85])
#
# cmap = mpl.cm.RdYlBu
...
fig.savefig(f'figures/simulations_{int(include_FLARES)}{int(add_current_surveys)}{int(add_future_surveys)}.pdf')
